...
stock = st.text_input('Enter stock symbol','MSFT')
...
mva_100=data.Close.rolling(100).mean()
...
plt.plot(data.Close, 'y', label='Closing price')
plt.plot(mva_50, 'r', label='Moving Average of 50 days')
plt.plot(mva_100, 'g', label='Moving Average of 100 days')
plt.plot(mva_200, 'b', label='Moving Average of 200 days')
...
st.subheader('The Root mean squared error of our model is')
st.write(mean_error)
...
tops1,bottoms1=directional_change(arr_mva,data.High,data.Low,0.02)    # MVA OR ORIGIANAL CHANGES
...
model2=lm(r'Stock extremes2.keras')
...
plt.xlabel('Stock Index')
plt.ylabel('TOPS and bottoms Price')
plt.legend()
st.pyplot(fig)
...
model3=lm(r'Stock days.keras')

predict_days = model3.predict(x3)
predict_days = predict_days * scale
y3 = y3 * scale

data_days_array=np.array(data_test_days)
data_days=data_days_array[0:len(predict_days)]


st.subheader('Days betweens extremes with respect to MVA_200 and Predicted')
fig = plt.figure(figsize=(10, 6))
plt.plot(predict_days, 'r', label='Predicted days between')
plt.plot(data_days, 'g', label='Original day between')
plt.xlabel('Stock Index')
...
r2_days = r2s(data_days, predict_days)
mean_error_days = (np.sqrt(mse(data_days, predict_days)))
